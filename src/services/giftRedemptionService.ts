...
export const processGiftRedemption = async (
  qrCode: string
): Promise<{
  success: boolean;
  message: string;
  ...
  try {
    ...
    if (customer.redeemedGift) {
      return {
        success: false,
        message: `Gift has already been redeemed at ${
          new Date(
            customer.redemptionTimeStamp?.seconds * 1000
          ).toLocaleString() || "an earlier time"
        }`,
        customer,
        redeemed: true,
      };
 ...
 */
export const clearGiftRedemptionStatus = async (
  customerId: string
): Promise<boolean> => {
  try {
...
export const exportToCSV = (registrations: Customer[]): void => {
  ...
  link.setAttribute(
    "download",
    `gift_redemption_records_${new Date().toISOString().split("T")[0]}.csv`
  );
